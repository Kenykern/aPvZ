...
#	get_node("ChunkNodes").add_sibling(layers[layer].chunks, layers[neu_layer_name].chunks, true)
	...
	for key in layers:
		...
		for x in maxium_chunk_size.x:
			for y in maxium_chunk_size.y:
				var paint_canvas_chunk = load("res://addons/Godoxel/PaintCanvasChunk.tscn").instantiate()
				paint_canvas_chunk.setup(region_size)
				paint_canvas_chunk.name = "C-%s-%s" % [x, y]
				paint_canvas_chunk.position = \
...
#		return
#	grid_size = grid
#	canvas_image_node.scale = Vector2(grid, grid)
#
#
...
func get_all_used_regions_in_canvas():
	...
	var chunk_pool = PackedVector2Array()
