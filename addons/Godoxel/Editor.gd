...
@onready var right_panel: Panel = find_child("RightPanel")
@onready var left_panel: Panel = find_child("LeftPanel")
@onready var layer_buttons: Control = find_child("LayerButtons")
@onready var paint_canvas_container_node = find_child("PaintCanvasContainer")
@onready var paint_canvas: GECanvas = find_child("Canvas")
@onready var canvas_background: TextureRect = find_child("CanvasBackground")
@onready var colors_grid = find_child("Colors")
@onready var debug_text = find_child("DebugTextDisplay")
@onready var preview_window: Control = find_child("PreviewWindow")
@onready var preview_layer_textures = find_child("PreviewLayerTextures")
@onready var shift_shortcut_window = find_child("ShortcutWindow")
@onready var frame_selection_container = find_child("FrameSelectionContainer")
@onready var anim_panel = find_child("AnimationPanel")
@onready var color_picker_button = find_child("ColorPicker")

var allow_drawing = true
var selected_color = Color(1, 1, 1, 1) : set = set_selected_color
...
func _ready():
	...
	#connect nodes
	#--------------------
	if not anim_panel.is_connected("on_add_animation",Callable(self,"_on_add_animation")):
		anim_panel.connect("on_add_animation",Callable(self,"_on_add_animation"))
		anim_panel.connect("on_add_frame_pressed",Callable(self,"_on_add_frame_pressed"))
		anim_panel.connect("on_frame_pressed",Callable(self,"_on_frame_pressed"))
		anim_panel.connect("on_animation_pressed",Callable(self,"_on_animation_pressed"))
		anim_panel.connect("on_animation_move",Callable(self,"_on_animation_move"))
		anim_panel.connect("on_animation_duplicated",Callable(self,"_on_animation_duplicated"))
		anim_panel.connect("on_animation_selected",Callable(self,"_on_animation_selected"))
		anim_panel.connect("on_animation_deleted",Callable(self,"_on_animation_deleted"))
		
	
	if not colors_grid.is_connected("color_change_request",Callable(self,"change_color")):
		colors_grid.connect("color_change_request",Callable(self,"change_color"))
	
	if not is_connected("visibility_changed",Callable(self,"_on_Editor_visibility_changed")):
		connect("visibility_changed",Callable(self,"_on_Editor_visibility_changed"))
	
	canvas_background.material.set_shader_parameter(
			"pixel_size", 8 * pow(0.5, big_grid_pixels)/paint_canvas.pixel_size)
	
	# ready
	set_tool(Tools.PAINT)
	
	find_child("BrushSizeLabel").text = str(int(find_child("BrushSize").value))
...
#	anim_stripe.connect("on_frame_pressed",Callable(self,"_on_frame_pressed"))
#
#	# data -> ui
...
#	#anim_panel._on_add_frame_pressed(0, 0) #ugly
	...
	# start thread
#	preview_thread.start(Callable(self,"_update_frame_preview"))
	...
	if event is InputEventMouseButton:
		if event.pressed and not event.is_echo():
			if is_mouse_in_preview_window():
				_preview_drag_start_pos = preview_layer_textures.get_child(0).position
				_preview_drag_pos = get_global_mouse_position()
			elif is_mouse_in_canvas():
				_middle_mouse_pressed_start_pos = paint_canvas.position
				...
				Tools.COLORPICKER:
					if event.button_index == MOUSE_BUTTON_LEFT:
						if event.pressed:
							...
							_picked_color = true
							find_child("Colors").add_color_prefab(selected_color)
...
func _process(delta):
	...
	canvas_position = paint_canvas.global_position
		...
		if is_mouse_in_canvas():
			if not paint_canvas.is_active_layer_locked():
				if is_position_in_canvas(get_global_mouse_position()) or \
					...
					print(cell_mouse_position, " not in ", paint_canvas_container_node.size)
					print("not in canvas")
...
func _handle_shortcuts(event: InputEvent):
	...
	if shift_pressed:
		if shortcuts[KEY_SHIFT].has(event.keycode):
			if shift_shortcut_window.check_input_for_shorcut(event, event.shift):
				if shortcuts[KEY_SHIFT].has(event.keycode):
					_handle_shortcut(shortcuts[KEY_SHIFT][event.keycode])
			accept_event()
			return
	elif ctrl_pressed:
		if shortcuts[KEY_CTRL].has(event.keycode):
			_handle_shortcut(shortcuts[KEY_CTRL][event.keycode])
			accept_event()
			return
	elif shortcuts.has(event.keycode):
			accept_event()
			_handle_shortcut(shortcuts[event.keycode])
			return
...
#					print(pixel)
		...
		Tools.BRUSH:
			var pixels = BrushPrefabs.get_brush(selected_brush_prefab, find_child("BrushSize").value)
			for pixel in pixels:
...
func load_project(data):
	...
	for anim_idx in range(animations.size()):
		if current_animation == null and not animations[anim_idx].frames.is_empty():
	...
	if is_preview_dragging():
		if Input.is_mouse_button_pressed(MOUSE_BUTTON_MIDDLE):
			...
			for child in preview_layer_textures.get_children():
				child.position = _preview_drag_start_pos
				child.position += get_global_mouse_position() - _preview_drag_pos
...
func _handle_canvas_scroll():
	if is_dragging():
		if Input.is_mouse_button_pressed(MOUSE_BUTTON_MIDDLE):
			if not is_dragging():
				return
			paint_canvas.position = _middle_mouse_pressed_start_pos
			paint_canvas.position += get_global_mouse_position() - _middle_mouse_pressed_pos
			if paint_canvas.position.y < -paint_canvas.size.y:
				paint_canvas.position.y = -paint_canvas.size.y
			if paint_canvas.position.x < -paint_canvas.size.x:
				paint_canvas.position.x = -paint_canvas.size.x
				
			if paint_canvas.position.x > global_position.x + size.x:
				paint_canvas.position.x = global_position.x + size.x
			
			if paint_canvas.position.y > global_position.y + size.y:
				paint_canvas.position.y = global_position.y + size.y
...
func _handle_preview_zoom(event: InputEvent):
	if event.is_pressed():
		if event.button_index == MOUSE_BUTTON_WHEEL_UP:
			...
			for child in preview_layer_textures.get_children():
				child.position -= child.get_local_mouse_position()
				child.position.x = clamp(child.position.x, 
					-child.size.x * 0.8, 
					size.x)
				child.position.y = clamp(child.position.y, 
					-child.size.y * 0.8, 
					size.y)
...
func _handle_canvas_zoom(event: InputEvent):
	if event.is_pressed():
		if event.button_index == MOUSE_BUTTON_WHEEL_UP:
			...
			find_child("CanvasBackground").material.set_shader_parameter(
					"pixel_size", 8 * pow(0.5, big_grid_pixels)/paint_canvas.pixel_size)
			paint_canvas.position -= paint_canvas.get_local_mouse_position()
			paint_canvas.position.x = clamp(paint_canvas.position.x, 
					-paint_canvas.size.x * 0.8, 
					size.x)
			paint_canvas.position.y = clamp(paint_canvas.position.y, 
					-paint_canvas.size.y * 0.8, 
					size.y)
			...
			find_child("CanvasBackground").material.set_shader_parameter(
					# 4 2 1
					"pixel_size", 8 * pow(0.5, big_grid_pixels)/paint_canvas.pixel_size)
			paint_canvas.position += paint_canvas.get_local_mouse_position() / 2
			paint_canvas.position.x = clamp(paint_canvas.position.x, 
					-paint_canvas.size.x * 0.8, 
					size.x)
			paint_canvas.position.y = clamp(paint_canvas.position.y, 
...
#			return
		...
		match brush_mode:
			...
			Tools.BRUSH:
				do_action([cell_mouse_position, last_cell_mouse_position, selected_color,
						selected_brush_prefab, find_child("BrushSize").value])
			Tools.LINE:
				do_action([cell_mouse_position, last_cell_mouse_position, selected_color])
			Tools.RECT:
				do_action([cell_mouse_position, last_cell_mouse_position, selected_color])
...
func _adjust_preview_layer_size():
	...
	while _preview_scale > min_preview_scale:
		if width * _preview_scale < preview_layer_textures.size.x:
			break
		set_preview_scale(_preview_scale / 2)
		
	while _preview_scale > min_preview_scale:
		if height * _preview_scale < preview_layer_textures.size.y:
			break
		set_preview_scale(_preview_scale / 2)
...
func _update_preview_layers_size():
	for child in preview_layer_textures.get_children():
		child.scale.x = _preview_scale
		child.scale.y = _preview_scale
...
func _center_element(element: Control):
	...
	element.offset_left = -width
	element.offset_top = -height
	element.offset_right = width
	element.offset_bottom = height
...
func highlight_layer(layer_name: String):
	if animations.is_empty() or animations[0].frames.is_empty():
		return
...
func _add_layer_button():
	var layer_button = LayerButton.instantiate()
	layer_buttons.add_child(layer_button, true)
	_total_added_layers += 1
	layer_button.find_child("Select").text = "Layer " + str(get_layer_count())
...
func _connect_layer_buttons():
	for layer_btn in layer_buttons.get_children():
		if layer_btn.find_child("Select").is_connected("pressed",Callable(self,"select_layer")):
			continue
		layer_btn.find_child("Select").connect("pressed",Callable(self,"select_layer").bind(layer_btn))
		layer_btn.find_child("Visible").connect("pressed", self, "toggle_layer_visibility",
				[layer_btn.find_child("Visible"), layer_btn.name])
		layer_btn.find_child("Up").connect("pressed",Callable(self,"move_down").bind(layer_btn))
		layer_btn.find_child("Down").connect("pressed",Callable(self,"move_up").bind(layer_btn))
		layer_btn.find_child("Lock").connect("pressed", self, "lock_layer",
...
func _on_ColorPicker_popup_closed():
	find_child("Colors").add_color_prefab(color_picker_button.color)
...
func is_any_menu_open() -> bool:
	return $ChangeCanvasSize.visible or \
			...
			find_child("Navbar").is_any_menu_open()


func _on_LockAlpha_pressed():
	var checked = find_child("LockAlpha").pressed
	paint_canvas.active_layer.toggle_alpha_locked()
	for i in range(find_child("Layer").get_popup().get_item_count()):
		if find_child("Layer").get_popup().get_item_text(i) == "Toggle Alpha Locked":
			find_child("Layer").get_popup().set_item_checked(i, 
					not find_child("Layer").get_popup().is_item_checked(i))
...
func _on_BrushSize_value_changed(value: float):
	find_child("BrushSizeLabel").text = str(int(value))
