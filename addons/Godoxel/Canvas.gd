...
@export var pixel_size: float = 16 : set = set_pixel_size
@export var canvas_width = 48 setget set_canvas_width # == pixels # (int, 1, 2500)
@export var canvas_height = 28 setget set_canvas_height # == pixels # (int, 1, 2500)
@export var grid_size = 16 : set = set_grid_size
@export var big_grid_size = 10 : set = set_big_grid_size
...
func _ready():
	...
	# Set nodes
	#-------------------------------
	canvas = find_child("Canvas")
	grid = find_child("Grid")
	big_grid = find_child("BigGrid")
	canvas_layers = find_child("CanvasLayers")
	...
	# setup layers and canvas
	#-------------------------------
	#canvas_size = Vector2(int(size.x / grid_size), int(size.y / grid_size))
...
func set_grid_size(size):
	grid_size = size
	if not find_child("Grid"):
		return
	find_child("Grid").size = size * pixel_size


func set_big_grid_size(size):
	big_grid_size = size
	if not find_child("BigGrid"):
		return
	find_child("BigGrid").size = size * pixel_size


func set_canvas_width(val: int):
	canvas_width = val
	size.x = canvas_width * pixel_size


func set_canvas_height(val: int):
	canvas_height = val
	size.y = canvas_height * pixel_size
...
func get_neighbouring_pixels(pos_x: int, pos_y: int) -> Array:
	...
	while not to_check_queue.is_empty():
