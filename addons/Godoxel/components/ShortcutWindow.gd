...
@onready var shortcut_container = find_child("ShortcutContainer")
...
func setup(shortcuts: Dictionary):
	...
	for panel in panels:
		...
		var shortcut = OS.get_keycode_string(shortcuts.keys()[idx])
...
func check_input_for_shorcut(event: InputEvent, shift_pressed: bool):
	...
	if event.pressed:
		if event.keycode in shortcuts.keys():
			for panel in panels:
				panel.deselect()
...
func get_panel(keycode: String):
	var character = keycode.substr(keycode.length() - 1, 1)
	for panel in panels:
